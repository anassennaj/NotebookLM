  ...
  return <div className="flex-1 flex flex-col h-full overflow-hidden">
      {hasSource ? <div className="flex-1 flex flex-col h-full overflow-hidden">
          ...
          <div className="p-4 border-b border-border flex-shrink-0">
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <h2 className="text-lg font-medium text-foreground">Discussion</h2>
              ...
              <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-muted rounded-lg p-6 mb-6">
                  {isGenerating ? <div className="flex items-center space-x-2 text-muted-foreground">
                    ...
                    </div> : <MarkdownRenderer content={notebook?.description || 'Aucune description disponible pour ce carnet.'} className="prose prose-invert max-w-none text-foreground leading-relaxed" />}
                </div>
                ...
                {(messages.length > 0 || pendingUserMessage || showAiLoading) && <div className="mb-6 space-y-4">
                    {messages.map((msg, index) => <div key={msg.id} className={`flex ${isUserMessage(msg) ? 'justify-end' : 'justify-start'}`}>
                        <div className={`${isUserMessage(msg) ? 'max-w-xs lg:max-w-md px-4 py-2 bg-[#0088c2] text-white rounded-lg' : 'w-full'}`}>
                          <div className={isUserMessage(msg) ? '' : 'prose prose-invert max-w-none text-foreground'}>
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex space-x-4">
                <div className="flex-1 relative">
                  <Input placeholder={getPlaceholderText()} value={message} onChange={e => setMessage(e.target.value)} onKeyDown={e => e.key === 'Enter' && !isChatDisabled && !isSending && !pendingUserMessage && handleSendMessage()} className="pr-12 bg-background text-foreground" disabled={isChatDisabled || isSending || !!pendingUserMessage} />
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-8 overflow-hidden">
          <div className="text-center mb-8">
            ...
            <h2 className="text-xl font-medium text-foreground mb-4">Ajoutez une source pour commencer</h2>
          ...
          <div className="w-full max-w-2xl">
            <div className="flex space-x-4">
              <Input placeholder="Téléchargez une source pour commencer" disabled className="flex-1 bg-background text-muted-foreground" />
      ...
      <div className="p-4 border-t border-border flex-shrink-0">
        <p className="text-center text-sm text-muted-foreground">Compucom LM peut être imprécis; veuillez vérifier ses réponses.</p>
      </div>
