      ...
      <div className="w-full bg-muted border-r border-border flex flex-col h-full overflow-hidden">
        <div className="p-4 border-b border-border flex-shrink-0">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-medium text-foreground cursor-pointer hover:text-muted-foreground" onClick={handleBackToSources}>
              Sources
            </h2>
    ...
    <div className="w-full bg-muted border-r border-border flex flex-col h-full overflow-hidden">
        ...
        <div className="p-4">
          {isLoading ? (
            <div className="text-center py-8">
              <p className="text-sm text-muted-foreground">Chargement des sources...</p>
            </div>
            ...
            <div className="space-y-4">
              {sources.map((source) => (
                <ContextMenu key={source.id}>
                  <ContextMenuTrigger>
                    <Card className="p-3 border border-border bg-card cursor-pointer hover:bg-card/80" onClick={() => handleSourceClick(source)}>
                      <div className="flex items-start justify-between space-x-3">
                        <div className="flex items-center space-x-2 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="text-sm text-card-foreground truncate block">{source.title}</span>
                          </div>
                        </div>
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-foreground mb-2">Les sources enregistrées apparaîtront ici</h3>
              <p className="text-sm text-muted-foreground mb-4">Cliquez sur Ajouter une source ci-dessus pour ajouter des PDF, du texte ou des fichiers audio.</p>
            </div>
          )}
        </div>
      </ScrollArea>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent className="bg-card text-card-foreground">
          <AlertDialogHeader>
            <AlertDialogTitle>Supprimer {selectedSource?.title}?</AlertDialogTitle>
            <AlertDialogDescription className="text-muted-foreground">
              Vous êtes sur le point de supprimer cette source. Cette action est irréversible.
