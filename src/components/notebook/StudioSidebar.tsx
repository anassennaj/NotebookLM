...
const StudioSidebar = ({
  ...
  useEffect(() => {
    ...
    const checkAndRefresh = async () => {
      if (checkAudioExpiry(notebook.audio_url_expires_at)) {
        console.log('Audio URL expired, initiating auto-refresh...');
  ...
  const getStatusDisplay = () => {
    if (isAutoRefreshing) {
      return {
        icon: null,
        text: "Actualisation de l'URL...",
        description: "Mise à jour de l'accès audio"
      };
    }
    ...
    if (currentStatus === 'generating' || isGenerating) {
      ...
      return {
        ...
        text: "Prêt à être lu",
        description: "Aperçu audio disponible"
      };
    }
    return null;
  };
      ...
      <div className="p-4 border-b border-border flex-shrink-0">
        ...
        <Card className="p-4 mb-4 border border-border bg-card">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-medium text-card-foreground">Aperçu audio</h3>
          </div>
            ...
            /> : <Card className="p-3 border border-border bg-card">
              ...
              {currentStatus !== 'generating' && !isGenerating && !isAutoRefreshing && <div className="flex items-center space-x-3 mb-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-medium text-card-foreground">Conversation approfondie</h4>
                    <p className="text-sm text-muted-foreground">Deux présentateurs</p>
                  </div>
                </div>}
              ...
              {/* Audio error div */}
              {audioError && <div className="flex items-center space-x-2 mb-3 p-2 bg-red-900/20 rounded-md">
                  <AlertCircle className="h-4 w-4 text-red-500" />
                  <div className="flex-1">
                    <p className="text-sm text-red-500">Audio indisponible</p>
                  </div>
                  <Button size="sm" variant="outline" onClick={handleAudioRetry} className="text-red-500 border-red-800 hover:bg-red-900/20">
              ...
              <div className="flex space-x-2">
                <Button size="sm" onClick={handleGenerateAudio} disabled={isGenerating || currentStatus === 'generating' || !hasProcessedSource || isAutoRefreshing} className="flex-1 text-white bg-[#0088c2] hover:bg-[#006a99]">
      ...
      <ScrollArea className="flex-1 h-full">
        <div className="p-4">
          {isLoading ? <div className="text-center py-8">
              ...
              {notes.map(note => <Card key={note.id} className="p-3 border border-border bg-card hover:bg-card/80 cursor-pointer" onClick={() => handleEditNote(note)}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center space-x-2 mb-1">
                        {note.source_type === 'ai_response' ? <Bot className="h-3 w-3 text-blue-500" /> : <User className="h-3 w-3 text-muted-foreground" />}
                        <span className="text-xs text-muted-foreground uppercase">
                          {note.source_type === 'ai_response' ? 'Réponse IA' : 'Note'}
                        </span>
                      </div>
                      <h4 className="font-medium text-card-foreground truncate">{note.title}</h4>
              ...
              <h3 className="text-lg font-medium text-foreground mb-2">Les notes enregistrées apparaîtront ici</h3>
              <p className="text-sm text-muted-foreground">
                Enregistrez un message de discussion pour créer une nouvelle note, ou cliquez sur Ajouter une note ci-dessus.
              </p>
            </div>}
        </div>
      </ScrollArea>
